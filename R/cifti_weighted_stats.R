...
#'
#' @export
cifti_weighted_stats <- function(cifti_in,
                                 operation = NULL,
                                 percentile = NULL,
  ...
  if (spatial_weights) args <- c(args, "-spatial-weights")
  ...
  if (!is.null(operation)) {
    args <- c(args, "-mean") # default
    if (operation == "STDEV") args[length(args)] <- "-stdev"
    if (operation == "SAMPSTDEV") args[length(args)] <- "-sampstdev"
    if (operation == "SUM") args[length(args)] <- "-sum"
    if (operation == "MEAN") args[length(args)] <- "-mean"
